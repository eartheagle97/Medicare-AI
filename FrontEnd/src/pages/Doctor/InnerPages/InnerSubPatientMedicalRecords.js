...
import InnerAddPatientMedicalRecords from './InnerAddPatientMedicalRecords'
import axios from 'axios'
import { useParams } from 'react-router-dom'

function InnerSubPatientMedicalRecords() {
    const { id } = useParams()
    const [patientMedicalRecords, SetPatientMedicalRecords] = useState('')
    useEffect(() => {
        axios.get(`/GetMedicalRecords/${id}`)
            .then(resData => {
                SetPatientMedicalRecords(resData.data)
            })
    }, [])
    console.log("patientMedicalRecords",patientMedicalRecords)
    return (
        <div className="card">
...
export default InnerSubPatientMedicalRecords